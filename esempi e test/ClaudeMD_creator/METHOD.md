...
> o di sicurezza importanti, o quando serve il razionale di una regola.
>
...
> in `WORK_LOG`.
>
> **Rimandi**: dove il testo dice **"PARTE 2"** o **"Profilo di piattaforma"**,
> intende la sezione **"Progetto"** e il **profilo scelto** in `CLAUDE.md`
> (menu in `profiles/`). Dove dice **"comando di Gate"**, è il campo **Gate** dei
> "Comandi del progetto" in `CLAUDE.md`.
...
- **Piattaforma/e target**: scelta deliberata, non lasciata implicita (→ §1.5 e
  Profilo in PARTE 2). Se è ancora **incognita**, scioglierla è il **primo nodo**
...
- Avere una **suite di test riutilizzabile** come *gate di regressione*, eseguita
  **prima** (baseline verde) e **dopo** ogni modifica. A regime, il "dopo" basta.
  La **baseline verde** è definita dal **comando di Gate** del progetto (→ PARTE 2),
...
- `CLAUDE.md` — direttive operative di progetto; `METHOD.md` — il metodo (questo
  file).
...
- `CLAUDE.md` (direttive operative di progetto) · `METHOD.md` (metodo).
...
**Contesto come artefatto.** Questo metodo (`METHOD.md`), `CLAUDE.md` e i prompt
rilevanti sono **versionati** e riusati: sono parte del progetto, non usa-e-getta.
...
- **La forma della matrice dipende dal Profilo di piattaforma** (→ PARTE 2):
...
store, firma, canali) si definisce nel **Profilo di piattaforma** (→ PARTE 2).
...
- **Diagnostica remota**: un modo **a prova di errore** per far attivare all'utente
  ...
  toggle in-app, crash reporter) dipende dal **Profilo di piattaforma** (→ PARTE 2).
