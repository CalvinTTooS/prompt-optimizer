...
export const FLOW_CODE_INSTRUCTIONS = `
          ...
          - Regole del formato:
            ...
            5. "Safety & Branching": nelle istruzioni operative, fai lavorare l'agente sempre su un branch separato (es. 'git checkout -b'), per ogni task e non solo per il primo, così il ramo principale resta ripristinabile se l'esecuzione va storta.
            ...
            9. "Logging & Memory": fai aggiornare 'WORK_LOG.md' e mantenere un loop di auto-miglioramento su 'lessons.md' (dopo ogni correzione dell'utente: annota una regola sintetica — cosa evitare o fare, col perché; rileggi 'lessons.md' a inizio sessione; promuovi le lezioni stabili in 'CLAUDE.md' e rimuovile da 'lessons.md', così resta memoria di lavoro e non un archivio).
            10. "Project Directives": inserisci come primo passo, prima di qualunque azione, l'istruzione di leggere 'CLAUDE.md', e di lasciarlo invariato. Contiene le direttive di progetto decise dall'utente: l'agente le riceve e le applica, non le rinegozia né le riscrive.
            11. "No Human Reminders": scrivi il prompt come se l'unico lettore fosse l'agente. Un promemoria rivolto all'utente resterebbe senza destinatario, perché nessuno lo legge durante l'esecuzione.
          `;
...
export const FLOW_GEMINI_INSTRUCTIONS = `
          ...
          - Regole del formato:
            1. "Pure Markdown Structure": struttura il file con heading \`##\` e bullet, senza tag XML di alcun tipo (né \`<role>\`, né \`<context>\`, né altri): Gemini CLI concatena questo file con altri file Markdown, e un tag verrebbe letto come contenuto invece che come struttura. Tieni ogni riga ad alto segnale: se rimuovendola l'agente non sbaglierebbe, va tolta.
            2. "Hierarchical Awareness": Gemini CLI carica e concatena i file gerarchicamente (globale \`~/.gemini/\` → root progetto → sottodirectory), con "closest file wins". Genera contenuto adatto al livello dichiarato dall'utente (se non dichiarato, assumi root di progetto) ed evita di ripetere regole che apparterrebbero a un livello superiore, perché verrebbero ereditate due volte.
            3. "Filename Neutrality": il nome del file è configurabile (\`context.fileName\`, può essere \`AGENTS.md\`). Riferisciti al file per il suo ruolo e non per il nome — evita formule come "questo GEMINI.md" — perché il contenuto deve restare valido anche se l'utente lo rinomina o lo aliasa.
            4. "AGENTS.md Interoperability": includi sezioni compatibili con lo standard aperto AGENTS.md: comandi build/test, code style, convenzioni di commit/PR.
            5. "Verifiable Specificity": in ogni punto in cui il file nomina un'operazione (build, test, lint, formattazione), dai il comando concreto ed eseguibile (es. \`npm test\`), perché l'agente lo lancia davvero e una frase come "scrivi codice pulito" o "testa le tue modifiche" non è eseguibile. Se l'input non dichiara la toolchain, scegli la convenzione più diffusa per quel linguaggio, dichiara l'assunzione in una riga (es. "assumo npm; sostituisci se il progetto usa un altro gestore") e fornisci comunque i comandi concreti sotto quell'assunzione: un comando dichiaratamente assunto è correggibile, una frase generica no.
            6. "Modularity (@import)": se l'input è lungo o copre più aree, proponi uno scheletro con import modulari \`@path/to/file.md\` verso sotto-file tematici, invece di un unico blob monolitico.
            7. "Reason-before-act": istruisci l'agente a pianificare ed elencare i file che modificherà prima di agire, e a fornire il contesto prima e l'istruzione specifica alla fine.
            8. "No Auto-reload Assumptions": scrivi istruzioni stabili, che non presuppongano riletture automatiche durante la sessione: dopo una modifica l'utente deve lanciare \`/memory refresh\` a mano, quindi un contenuto che si aspetta un aggiornamento automatico resterebbe disatteso.
            9. "Self-improvement Memory": Prevedi un loop di auto-miglioramento su file \`lessons.md\`: dopo ogni correzione dell'utente l'agente annota una regola sintetica (cosa evitare/fare + perché) e rilegge \`lessons.md\` a inizio sessione; le lezioni stabili vanno promosse nel file di istruzioni principale e rimosse da \`lessons.md\`. Riferisciti al file di istruzioni per ruolo, non per nome (vedi regola 3).
            10. "No Human Reminders": scrivi il file come se l'unico lettore fosse l'agente. Un promemoria rivolto all'utente resterebbe senza destinatario, perché nessuno lo legge durante l'esecuzione.
          `;
